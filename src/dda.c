...
/*                                                   +#+                      */
/*   Created: 2026/02/18 14:53:04 by jdong         #+#    #+#                 */
/*   Updated: 2026/02/18 14:53:07 by jdong         ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_dda(t_dda *dda, t_env *env, t_ray ray)
{
	...
	{
		dda->stepx = 1;
		dda->distx = (dda->mapx + 1.0 - env->player.x)
			* cal_unit_dist(ray, 'x');
	}
	if (ray.diry < 0)
	{
	...
	{
		dda->stepy = 1;
		dda->disty = (dda->mapy + 1.0 - env->player.y)
			* cal_unit_dist(ray, 'y');
	}
}
